...
<b class="complaintsname" style="display:none">${complaintsname }</b>
   ...
   <div class="toold">
    	<form action="cablecomplaints" method="post" class="form-search">  
		 	住户,房屋号：<input type="text" value="${complaintsname }" class="cablecomplaints" name="cablecomplaints">  
 ...
 <div style="height: 547px;width: 100%;">
    ...
    <tbody class="complaints">
    	<c:forEach items="${complaint }" var="complaint">
		    	...
		    	<c:if test="${empty complaint.describe }">-</c:if>
			    <c:if test="${not empty complaint.describe }">${complaint.describe }</c:if>
		    	</td>
		    	<td>暂无</td>
		    	<td>${complaint.time_s }</td>
		    	<td>
			    	<c:if test="${empty complaint.time_e }">-</c:if>
			    	<c:if test="${not empty complaint.time_e }">${complaint.time_e }</c:if>
		    	</td>
		    	<td>
			    	<c:if test="${complaint.type_t !=0}">处理完毕</c:if>
			    	<c:if test="${complaint.type_t ==0}">等待处理</c:if>
		    	</td>
		    </tr>
    	</c:forEach>
    </tbody>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/assets/js/complainttable.js"></script>
</body>
</html>
